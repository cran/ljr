ljrf2 <- function(y,n,tm,X=NULL,ofst=0,R=1000,alpha=0.05)
{
 N=length(y)
 m=ncol(X)
 if (is.null(m)) 
 ...
 if (length(ofst)==1)
  ...
  ofst=as.double(ofst) 
 out=.C("forward2",as.double(y),as.double(n),as.double(tm),as.double(X),ofst,N,m,as.integer(R),p=double(2),as.double(alpha),nulls=integer(3),alts=integer(2),PACKAGE="ljr")
 cat("Forward algorithm for determining the number of joinpoints:\n")
 for (i in 1:(min(out$nulls[3],1)+1)){
...
as.double(ofst),beta=double(m+1),gamma=double(1),
N,m,ans=double(1),PACKAGE="ljr")
  cat('Model:\n')
