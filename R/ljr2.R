ljr2 <- function(y,n,tm,X=NULL,ofst=0)
{
   N=length(y)
   m=ncol(X)
   if (is.null(m)) 
   ...
   if (length(ofst)==1)
    ...
    ofst=as.double(ofst)
   out <- .C("ljr2",as.double(y),as.double(n),as.double(tm),as.double(X),ofst,beta=double(m+1),gamma=double(3),tau=double(2),N,m,ans=double(1),PACKAGE="ljr")
   cat('Model:\n')
