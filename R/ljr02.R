ljr02 <- function(y,n,tm,X=NULL,ofst=0,R=1000,alpha=0.05)
{
 N=length(y)
 m=ncol(X)
 if (is.null(m)) 
 ...
 if (length(ofst)==1)
  ...
  ofst=as.double(ofst)
 out1 <- .C("test02",as.double(y),as.double(n),as.double(tm),as.double(X),ofst,N,m,as.integer(R),p=double(1),PACKAGE="ljr")
 cat("Testing H0: no joinpoints vs. H1: 2 joinpoints\n")
 cat("p-value=",out1$p,"\n")
 if (out1$p>alpha){
  ...
  out2 <- .C("ljr2",as.double(y),as.double(n),as.double(tm),as.double(X),ofst,beta=double(m+1),gamma=double(3),tau=double(2),N,m,ans=double(1),PACKAGE="ljr")
  cat('Model:\n')
  cat('y~Binom(n,p) where p=invlogit(eta)\n')  
  if (m>0){
   if (is.null(dimnames(X)[[2]])==FALSE)
