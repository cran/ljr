ljr12 <- function(y,n,tm,X=NULL,ofst=0,R=1000,alpha=0.05)
{
 N=length(y)
 m=ncol(X)
 if (is.null(m)) 
 ...
 if (length(ofst)==1)
  ...
  ofst=as.double(ofst) 
 out1 <- .C("test12",as.double(y),as.double(n),as.double(tm),as.double(X),ofst,N,m,as.integer(R),p=double(1),PACKAGE="ljr")
 cat("Testing H0: 1 joinpoint vs. H1: 2 joinpoints\n")
 cat("p-value=",out1$p,"\n")
 if (out1$p>alpha){
